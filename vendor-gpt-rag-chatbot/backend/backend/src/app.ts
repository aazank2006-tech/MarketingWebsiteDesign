...
import kbData from "../kb/knowledge_base.json" with { type: "json" };
import { GroqChatClient, GroqClientError } from "./rag/groqClient.js";
import { TfidfRetriever, type Chunk } from "./rag/retriever.js";
import { RagChatService } from "./rag/service.js";

// A real static import (not a runtime fs.readFileSync call) so bundlers --
// tsx's esbuild locally, tsc for the Docker build, and Vercel's function
// bundler -- all resolve and embed this file the standard, well-supported
// way. This sidesteps a real bug we hit: Vercel's own build step tried to be
// "helpful" and auto-convert a readFileSync-based load into a generated
// import, but emitted it without the `type: "json"` attribute Node's ESM
// loader now requires, crashing with ERR_IMPORT_ATTRIBUTE_MISSING. Writing
// the import ourselves, correctly, avoids relying on that conversion at all.
...
const retriever = new TfidfRetriever(kbData as Chunk[]);
