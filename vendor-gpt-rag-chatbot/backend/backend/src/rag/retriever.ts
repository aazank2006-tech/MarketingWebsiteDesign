 ...
 *
 * Takes already-loaded chunk data (not a file path) so callers control how
 * the KB is loaded -- a static JSON import in app.ts, which works
 * identically and reliably across local dev, Docker, and Vercel's
 * serverless bundler, unlike a runtime fs.readFileSync() call.
 */
...
export class TfidfRetriever implements Retriever {
  ...
  private load(chunks: Chunk[]): void {
    if (!chunks.length) {
  ...
  /** Re-fit against a fresh set of chunks (e.g. after editing the KB). */
  reload(chunks: Chunk[]): void {
    this.load(chunks);
  }
}
